...
def build_target_tables() -> None:
    ...
    _expr = eval("when(col('activity_type').str.contains(r'(?i)supplied')).then(pl.lit('Supplied as goods')).otherwise(when(col('activity_type').str.contains(r'(?i)import')).then(pl.lit('Imported')).otherwise(when(col('activity_type').str.contains(r'(?i)export')).then(pl.lit('Exported')).otherwise(null)))", _NS, _local)
    df = df.with_columns(_expr.alias('activity'))

    _local = {'weight': df['weight'], 'unit': df['unit']}
    _expr = eval("when(col('unit').str.contains(r'(?i)kg')).then(col('weight') / 1000).otherwise(col('weight'))", _NS, _local)
