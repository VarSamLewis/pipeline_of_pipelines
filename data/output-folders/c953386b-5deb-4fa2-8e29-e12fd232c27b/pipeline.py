...
def build_target_tables() -> None:
    ...
    _expr = eval("when(col('period').str.contains(r'(?i)Q1')).then(pl.lit('2024-Q1')).otherwise(when(col('period').str.contains(r'(?i)Q2')).then(pl.lit('2024-Q2')).otherwise(when(col('period').str.contains(r'(?i)Q3')).then(pl.lit('2024-Q3')).otherwise(null)))", _NS, _local)
    ...
    _expr = eval("when(col('packaging_type').str.contains(r'(?i)primary')).then(pl.lit('Primary')).otherwise(when(col('packaging_type').str.contains(r'(?i)secondary')).then(pl.lit('Secondary')).otherwise(when(col('packaging_type').str.contains(r'(?i)shipment')).then(pl.lit('Shipment')).otherwise(when(col('packaging_type').str.contains(r'(?i)tertiary')).then(pl.lit('Tertiary')).otherwise(null))))", _NS, _local)
    df = df.with_columns(_expr.alias('packaging_class'))

    _local = {'activity_type': df['activity_type']}
    _expr = eval("when(col('activity_type').str.contains(r'(?i)supplied')).then(pl.lit('Supplied as goods')).otherwise(when(col('activity_type').str.contains(r'(?i)import|imported')).then(pl.lit('Imported')).otherwise(when(col('activity_type').str.contains(r'(?i)export|exported')).then(pl.lit('Exported')).otherwise(null)))", _NS, _local)
    df = df.with_columns(_expr.alias('activity'))

    _local = {'weight': df['weight'], 'unit': df['unit']}
    _expr = eval("when(col('unit').str.contains(r'(?i)kg')).then(col('weight') / 1000).otherwise(col('weight')).cast(Float64)", _NS, _local)
